...
class DDPG(BaseAgent):
    ...
    def __init__(self, task):
        ...
        print("observation_space: high={}, low={}, range={}\naction_space: high={}, low={}, range={}".format(
            self.task.observation_space.high, self.task.observation_space.low, self.state_range,
            self.task.action_space.high, self.task.action_space.low, self.action_range))

        # Policy parameters
        self.w = np.random.normal(
            size=(self.state_size, self.action_size),  # weights for simple linear policy: state_space x action_space
            scale=(self.action_range / (2 * self.state_size)).reshape(1, -1))  # start producing actions in a decent range
        print("init self.w => {}".format(self.w))
